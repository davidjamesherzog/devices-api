...
export class DevicesRepository extends Repository<Devices> {
  ...
  async addDevice(addDevicesDTO: AddDevicesDto, network: Networks) : Promise<Devices> {
    const {
      name,
      description,
      os,
    ...
    device.os = os;
    device.dhcp = (dhcp === 'true') ? true : false;
  ...
  async updateDevice(updateDevicesDTO: UpdateDevicesDto, device: Devices, network: Networks) : Promise<Devices> {
    const {
      name,
      description,
      os,
