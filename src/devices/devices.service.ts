...
export class DevicesService {
  ...
  async add(addDevicesDTO: AddDevicesDto) : Promise<Devices> {
    const network: Networks = await this.networksService.findById(addDevicesDTO.networkId);
    return await this.devicesRepository.addDevice(addDevicesDTO, network);
  }

  async findById(id: number) : Promise<Devices> {
    const device: Devices = await this.devicesRepository.findOne(id, { relations: ['network'] });
  ...
  async update(id: number, updateDevicesDTO: UpdateDevicesDto) : Promise<Devices> {
    const network: Networks = await this.networksService.findById(updateDevicesDTO.networkId);
    const device: Devices = await this.findById(id);
    return await this.devicesRepository.updateDevice(updateDevicesDTO, device, network);
  }
